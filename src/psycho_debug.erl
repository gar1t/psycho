...
trace_module(Module, Options) ->
    Opts = psycho_opt:validate(Options, ?OPTIONS_SCHEMA),
    start_tracer(Opts),
    dbg_tpl(Module, Opts),
    dbg_calls().
...
message_flags(Opts, Acc) ->
    case psycho_opt:value(send_only, Opts) of
        true -> [s|Acc];
        false ->
            case psycho_opt:value(receive_only, Opts) of
                true -> [r|Acc];
                false -> [m|Acc]
            end
    end.
...
process_event_flags(Opts, Acc) ->
    case psycho_opt:value(process_events, Opts) of
        true -> [p|Acc];
        false -> Acc
    end.
...
tracer(Opts) ->
    Pattern = pattern_match_spec(psycho_opt:value(pattern, Opts)),
    Out = trace_device(psycho_opt:value(file, Opts)),
...
match_spec(Opts) ->
    case psycho_opt:value(no_return, Opts) of
	true ->
	    [{'_', [], []}];
	false ->
	    [{'_', [], [{return_trace}]}]
